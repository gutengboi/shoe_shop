...
class PaymentHandler: NSObject {
    ...
    func shippingMethodCalculator() -> [PKShippingMethod] {
        ...
        if let shippingEnd = shippingEnd, let shippingStart = shippingStart {
            _ = calendar.dateComponents([.calendar, .year, .month, .day], from: shippingStart)
            _ = calendar.dateComponents([.calendar, .year, .month, .day], from: shippingEnd)
            
            let shippingDelivery = PKShippingMethod(label: "Delivery", amount: NSDecimalNumber(string: "0.00"))
//            shippingDelivery.dateComponentsRange = PKDateComponentsRange(start: startComponents, end: endComponents)
            shippingDelivery.detail = "Shoes sent to your address"
    ...
    func startPayment(products: [Product], total: Int, completion: @escaping PaymentCompletionHandler) {
        ...
        paymentRequest.merchantIdentifier = "merchant.io.anikala.ecomDemo"
